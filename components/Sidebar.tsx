...
import { 
  ...
  FileInput, LogOut, ClipboardList, Bell, X, Eraser, Activity, Calendar as CalIcon, Info, Trash2, RefreshCw, CheckCircle, BarChart2
} from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggle, onLogout }) => {
  ...
  useEffect(() => {
    // LÓGICA ATUALIZADA PARA RESOLVER O NOME (TIAGO AO INVÉS DE 81025901)
    const storedLogin = localStorage.getItem('safemaint_user') || '';
    const storedRole = localStorage.getItem('safemaint_role') || 'OPERADOR';
    
    const resolvedName = StorageService.resolveUserName(storedLogin);
    
    setDisplayName(resolvedName);
    setDisplayInitial(resolvedName ? resolvedName.charAt(0).toUpperCase() : '?');
  ...
  const sections = [
    { title: "OPERAÇÕES", items: [
      ...
      { path: '/availability', label: 'Indicadores', icon: <BarChart2 size={20} /> }, // NOVO ITEM
    ]},
    ...
    // EXECUÇÃO: Apenas para ADMIN
    ...(userRole === 'ADMIN' ? [{ title: "EXECUÇÃO", items: [
    ...
    { title: "SISTEMA", items: [
      ...
      // LIXEIRA E CONFIGURAÇÕES: Apenas para ADMIN
      ...(userRole === 'ADMIN' ? [
        ...
        <div className="h-28 flex items-center justify-center p-3 border-b border-gray-800 relative shrink-0">
            ...
            <div className="absolute top-2 left-2 z-50" ref={dropdownRef}>
                <button 
                    ...
                    title="Notificações"
                >
                    <Bell size={24} className={hasUrgent ? 'animate-swing text-red-500' : ''} />
            ...
            {/* LOGO PROFISSIONAL (SEM CUBO 3D) */}
            <div className={`transition-all duration-500 flex justify-center items-center ${isCollapsed ? 'scale-75' : 'scale-100'}`}>
