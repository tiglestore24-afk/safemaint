...
import { ArrowLeft, FileText, Eye, CheckCircle, X, AlertCircle, ShieldCheck, ChevronRight, Loader2 } from 'lucide-react';
...
export const ARTAtividade: React.FC = () => {
  ...
  const [isSaving, setIsSaving] = useState(false); // Estado de salvamento visual
  ...
  return (
    <div className="max-w-7xl mx-auto pb-24 px-4 md:px-6 relative">
      <div className="flex items-center gap-3 mb-8 border-b border-gray-200 pb-4 pt-4">
        <button onClick={() => navigate(-1)} className="mr-2 p-2 rounded-full hover:bg-gray-200 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <section className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
                  ...
                  <div className="bg-gray-50 px-6 py-3 border-b border-gray-200 flex justify-between items-center">
                      <div className="flex items-center gap-3">
                          <div className="bg-blue-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-black text-sm shadow-md">1</div>
                          <h3 className="font-bold text-gray-800 uppercase text-sm tracking-wide">DADOS DA MANUTENÇÃO</h3>
                      </div>
                      {header.om && header.tag ? <CheckCircle size={18} className="text-green-500" /> : <div className="w-4 h-4 rounded-full border-2 border-gray-300"></div>}
                  </div>
                  <div className="p-6">
              ...
              <section className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
                  <div className="absolute top-0 left-0 w-1 h-full bg-green-500"></div>
                   <div className="bg-gray-50 px-6 py-3 border-b border-gray-200 flex justify-between items-center">
                      <div className="flex items-center gap-3">
                          <div className="bg-green-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-black text-sm shadow-md">2</div>
                          <h3 className="font-bold text-gray-800 uppercase text-sm tracking-wide">DEFINIÇÃO DE RISCO (ART PADRÃO)</h3>
                      </div>
                      {isConfirmed ? <CheckCircle size={18} className="text-green-500" /> : <div className="w-4 h-4 rounded-full border-2 border-gray-300"></div>}
                  </div>
                  ...
                  <div className="p-6">
                      <div className="mb-4">
                          ...
                          <select 
                              className="w-full border-2 border-gray-300 p-4 rounded-lg bg-gray-50 focus:ring-2 focus:ring-green-500 focus:border-green-500 font-bold text-gray-800 outline-none transition-all" 
                          ...
                          <div className="animate-fadeIn">
                              <div className="flex flex-col md:flex-row gap-4 items-center bg-blue-50 p-4 rounded-lg border border-blue-100 mb-4">
                                  <div className="flex-1">
                                      <h4 className="font-black text-blue-900 text-sm">VOCÊ SELECIONOU:</h4>
                                      <p className="text-xs font-bold text-blue-700 mt-1">{selectedART.taskName}</p>
                                  </div>
                                  <button 
                                      onClick={handlePreviewClick}
                                      className={`px-6 py-3 rounded-lg font-black shadow-md flex items-center justify-center gap-2 transition-transform active:scale-95 border-b-4 w-full md:w-auto ${isConfirmed ? 'bg-green-100 text-green-700 border-green-300' : 'bg-blue-600 text-white border-blue-800 hover:bg-blue-700'}`}
                                  >
                                      {isConfirmed ? (
          ...
          <div className="lg:col-span-1">
              <div className="sticky top-6 space-y-4">
                  <div className="bg-gray-800 text-white rounded-xl shadow-lg p-6 border-b-4 border-green-500">
                      <h3 className="font-black text-lg mb-4 flex items-center gap-2">
                          <ShieldCheck className="text-green-400" /> STATUS
                      </h3>
                      ...
                      <div className="space-y-4">
                          <div className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${header.om && header.tag ? 'bg-green-900/30 border-green-700/50 text-green-100' : 'bg-gray-700 border-gray-600 text-gray-400'}`}>
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${header.om && header.tag ? 'border-green-400 bg-green-500 text-white' : 'border-gray-500'}`}>
                          ...
                          <div className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${selectedART ? 'bg-green-900/30 border-green-700/50 text-green-100' : 'bg-gray-700 border-gray-600 text-gray-400'}`}>
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${selectedART ? 'border-green-400 bg-green-500 text-white' : 'border-gray-500'}`}>
                          ...
                          <div className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${isConfirmed ? 'bg-green-900/30 border-green-700/50 text-green-100' : 'bg-gray-700 border-gray-600 text-gray-400'}`}>
                               <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${isConfirmed ? 'border-green-400 bg-green-500 text-white' : 'border-gray-500'}`}>
                          ...
                          <div className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${signatures.length > 0 ? 'bg-green-900/30 border-green-700/50 text-green-100' : 'bg-gray-700 border-gray-600 text-gray-400'}`}>
                               <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${signatures.length > 0 ? 'border-green-400 bg-green-500 text-white' : 'border-gray-500'}`}>
          ...
          <div className="mt-8 animate-fadeIn">
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
                  <div className="absolute top-0 left-0 w-1 h-full bg-purple-500"></div>
                   <div className="bg-gray-50 px-6 py-3 border-b border-gray-200 flex items-center gap-3">
                      <div className="bg-purple-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-black text-sm shadow-md">3</div>
              ...
              <div className="fixed bottom-6 right-6 z-50">
                <button 
                    ...
                    className={`px-8 py-4 rounded-full shadow-xl font-black text-lg flex items-center justify-center gap-3 transform transition hover:scale-105 border-4 border-white ${signatures.length > 0 ? 'bg-green-600 hover:bg-green-700 text-white cursor-pointer' : 'bg-gray-400 text-gray-200 cursor-not-allowed'}`}
                >
                    SALVAR / INICIAR ATIVIDADE <ChevronRight size={24} />
                </button>
              </div>
          </div>
      )}
      ...
      {isSaving && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm">
              <div className="bg-white p-6 rounded-xl shadow-2xl flex flex-col items-center animate-bounce-in max-w-sm w-full">
                  <div className="relative mb-4">
                      <div className="w-16 h-16 rounded-full border-4 border-gray-200"></div>
                      <div className="absolute top-0 left-0 w-16 h-16 rounded-full border-4 border-[#10b981] border-t-transparent animate-spin"></div>
                  </div>
                  <h3 className="text-xl font-black text-gray-800 mb-1">INICIANDO ATIVIDADE</h3>
                  <p className="text-xs font-bold text-gray-500 text-center uppercase">Registrando ART, assinaturas e iniciando contagem de tempo...</p>
              </div>
          </div>
      )}
