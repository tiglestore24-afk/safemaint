...
export const StorageService = {
  // --- USER NAME RESOLVER ---
  resolveUserName: (loginOrMatricula: string): string => {
      if (!loginOrMatricula) return 'SISTEMA';
      const cleanLogin = loginOrMatricula.trim().toUpperCase();
      
      const users = StorageService.getUsers();
      const user = users.find(u => u.login === cleanLogin || u.matricula === cleanLogin);
      if (user) return user.name.toUpperCase();

      const employees = StorageService.getEmployees();
      const emp = employees.find(e => e.matricula === cleanLogin);
      if (emp) return emp.name.toUpperCase();
  ...
  startMaintenance: async (task: ActiveMaintenance) => {
      ...
      // BLOQUEIO DE DUPLICIDADE: Verifica se a OM já está em execução
      if (task.omId) {
          const alreadyRunning = active.some(t => t.omId === task.omId && t.id !== task.id);
          if (alreadyRunning) {
              throw new Error("Esta OM já possui uma atividade em execução!");
          }
      }

      // REMOVE EXISTENTE SE HOUVER (PARA UPDATES) E ADICIONA O NOVO
      const updated = [task, ...active.filter(t => t.id !== task.id)];
      ...
      // ATUALIZA STATUS DA OM (TRAVA) E GARANTE QUE ELA SAIA DA FILA DE PENDENTES
      if (task.omId) {
          const oms = StorageService.getOMs();
          const om = oms.find(o => o.id === task.omId);
          if (om) {
              om.status = 'EM_ANDAMENTO';
              await StorageService.saveOM(om); // This triggers OMS update event, refreshing OM Management list
          }
      }
  ...
  setMaintenancePartial: async (id: string) => {
      ...
      const task = active.find(t => t.id === id);
      if (task) {
          // 1. Calcula tempo da sessão atual
          let sessionDuration = 0;
          if (task.status === 'ANDAMENTO' && task.currentSessionStart) {
              const now = new Date();
              const start = new Date(task.currentSessionStart);
              sessionDuration = now.getTime() - start.getTime();
          }
          
          // 2. Calcula tempo total acumulado para ESTE turno
          const totalDurationForThisUser = (task.accumulatedTime || 0) + sessionDuration;

          // 3. Salva HISTÓRICO para o usuário atual
          const h = Math.floor(totalDurationForThisUser / 3600000);
          const m = Math.floor((totalDurationForThisUser % 3600000) / 60000);
          ...
          const log: MaintenanceLog = {
              ...
              startTime: task.startTime, // Início original deste turno (aproximado)
              endTime: new Date().toISOString(),
              duration: durationStr,
              responsible: task.openedBy || 'SISTEMA',
              status: 'PARCIAL (TURNO)',
              type: task.origin
          };
          await StorageService.saveHistory([log, ...StorageService.getHistory()]);

          // 4. PREPARA PRO PRÓXIMO: Reseta o tempo acumulado e muda status
          task.accumulatedTime = 0; // ZERA PARA O PRÓXIMO
          task.status = 'AGUARDANDO'; // Libera Lock e muda cor no painel
          task.currentSessionStart = undefined;
